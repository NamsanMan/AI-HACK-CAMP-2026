...
from utils.score_smoothing import RiskHysteresis, ScoreSmoother, risk_color_bgr
...
def parse_args():
    ...
    parser.add_argument("--display-interval-sec", type=float, default=0.5)
...
def draw_score(frame, bbox, raw_score, display_score, state, frame_idx):
    if bbox is not None:
        x1, y1, x2, y2 = bbox
        cv2.rectangle(frame, (x1, y1), (x2, y2), (40, 220, 255), 2)
    color = risk_color_bgr(state)
    cv2.rectangle(frame, (12, 12), (380, 100), (0, 0, 0), -1)
    cv2.putText(frame, f"Risk: {display_score:.2f}", (24, 46), cv2.FONT_HERSHEY_SIMPLEX, 0.9, color, 2, cv2.LINE_AA)
    cv2.putText(frame, f"Status: {state}", (24, 74), cv2.FONT_HERSHEY_SIMPLEX, 0.65, color, 2, cv2.LINE_AA)
    cv2.putText(frame, f"raw {raw_score:.3f}  frame {frame_idx}", (24, 94), cv2.FONT_HERSHEY_SIMPLEX, 0.45, (220, 220, 220), 1, cv2.LINE_AA)
...
def main():
    ...
    display_score = 0.0
    display_state = "Low"
    smoother = ScoreSmoother(args.ema_alpha)
    hysteresis = RiskHysteresis()
    display_every = max(1, int(round((fps or 30) * args.display_interval_sec)))
    frame_idx = 0
    scores = []
        ...
        while True:
            ...
            if frame_idx % max(1, args.sample_every) == 0 and bbox is not None:
                ...
                if crop is not None:
                    ...
                    smoothed = smoother.update(last_score)
                    if frame_idx % display_every == 0:
                        display_score = smoothed
                        display_state = hysteresis.update(display_score)

            draw_score(frame, bbox, last_score, display_score, display_state, frame_idx)
